...
class MasterAuthController extends Controller
{
    ...
    public function login(MasterLoginRequest $request): RedirectResponse|JsonResponse
    {
        $credentials = $request->only('email', 'password');
        $remember = $request->boolean('remember');

        if (! Auth::attempt($credentials, $remember)) {
            $this->activityLogger->log('auth.master.login_failed', null, [
        ...
        if (! $user->hasRole('master_admin')) {
            ...
            $this->activityLogger->log('auth.master.login_denied_role', null, [
        ...
        $this->activityLogger->log('auth.master.login_success', $user, [], $request);

        $target = route('master.dashboard');
